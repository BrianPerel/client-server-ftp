 ...
 * Server FTP program
 *
 * cmds are analyzed by this server ftp program
 *
 * You must run the server ftp program before running client. Otherwise the connection from client will fail since server is not running (listening for connection)
 * The list of ftp commands: gets, puts, mkdir, user, pass, mkdir, rmdir, pwd, stat, help, quit, cd, ls, rm, send, recv
...
/* Error and OK constants codes with values, these values are returned if something goes wrong in the program*/
#define OK 0
...
int main(int argc, char *argv[]) {
	...
	do {
		...
		char *users[20] = { "abc", "xyz", "Brian", "John", "Amy", "Emily",
		...
		char *pass[20] = { "bananas", "monkeys", "friday", "book",
				...
				"Calisthenics", "Dr. Dre", "The1letterPassword", "MyPassword",
				"\0" };
					...
					if (status != 0) {
						...
						do {
							status = receiveMessage(dcSocket, buffer,
									sizeof(buffer), &msgSize); /* receive message from client and write content to txt file created, while msgSize > 0 */
		...
		if (userCheck == true && passCheck == true && cmdCheck == false) {
			strcpy(replyMsg, "Invalid FTP cmd\n");
				...
				&& (strcmp(cmd, "stat") != 0) && (strcmp(cmd, "user") != 0)) {
		 ...
		 */
		status = sendMessage(ccSocket, replyMsg, strlen(replyMsg) + 1); /* Added 1 to include NULL character */
...
int clntConnect(char *serverName, int *s) {
	...
	memcpy((char*) &serverAddress.sin_addr, serverIPstructure -> h_addr, serverIPstructure -> h_length);
 ...
 * msgSize	- # of bytes (size) of the received msg in octet/bytes (output)
 *
 * Return status
